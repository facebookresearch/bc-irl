...
from imitation_learning.common.distributions import (FixedCategorical,
                                                     FixedNormal)
...
from torchrl.modules import (ActorValueOperator, ConvNet, ProbabilisticActor,
                             TensorDictModule, TensorDictSequential,
                             ValueOperator)
...
class Policy(nn.Module):
    def __init__(
        ...
        actor.apply(partial(init_weights, gain=np.sqrt(2)))
        critic.apply(partial(init_weights, gain=np.sqrt(2)))
